...
function ChatRoom() {
  ...
  // ── Refs ─────────────────────────────────────────────────
  const pendingConfirmedMsg = useRef(null);
  ...
  const { typeText } = useTypeMessage(
    useCallback(() => {
      ...
      if (pendingConfirmedMsg.current) {
        const msg = pendingConfirmedMsg.current;
        pendingConfirmedMsg.current = null;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await confirmChat(sessionId, reply);
      const aiMessageId = res?.messageId ?? res?.id ?? null;
      pendingConfirmedMsg.current = { role: "ai", content: reply, ...(aiMessageId && { id: aiMessageId }) };
      setReplies([reply]);
      setReplyIdx(0);
      setIsTyping(true);
      ...
      setInput(input);
      toast.error(error.response?.data || "메시지 전송에 실패했어요. 다시 시도해주세요.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      // 유저 메시지 편집 → AI 응답 생성 + 자동 확정 / AI 메시지 편집 → DB 저장만
      if (editingRole === "user") {
        setIsRegenerating(true);
        try {
          const { reply } = await regenerateChat(sessionId);
